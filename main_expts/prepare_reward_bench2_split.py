#!/usr/bin/env python3
"""
Prepare a fixed random split of RewardBench-2 for diagnostic chosen-SFT runs.

This script:
1. Loads `allenai/reward-bench-2` (or another similarly-shaped dataset).
2. Shuffles with a fixed seed.
3. Builds a fixed train/heldout split over prompts.
4. Writes:
...
Example:
    conda run -n rlmt python prepare_reward_bench2_split.py \
        --train_size 1500 \
        --heldout_size 365 \
        --output_dir data/reward_bench2_diag_split
"""
