...
class DiffuGRPOTrainer(GRPOTrainer):
    ...
    def __init__(
        ...
        # TRL < 0.25 doesn't initialize _textual_logs in GRPOTrainer.__init__;
        # initialize it here so _generate_and_score_completions can always write to it.
    ...
    # -----------------------------------------------------------------------
    # Override: _get_per_token_logps_and_entropies (called by TRL _compute_loss)
    # -----------------------------------------------------------------------
    ...
    def _get_per_token_logps_and_entropies(
        ...
        **_kwargs,  # absorb VLM kwargs (pixel_values, image_grid_thw, etc.)
    ):
        """
        Route TRL's loss-step log-prob query through the diffusion forward process.

        TRL 0.24.0 calls this method (not _get_per_token_logps) inside _compute_loss.
        Without this override, it would use TRL's default AR computation, which
        mismatches the diffusion-masked old/ref log-probs precomputed in
        _generate_and_score_completions and causes KL explosion when beta > 0.

        Entropy is returned as zeros — it is used only for logging and the optional
        top_entropy_quantile masking (default 1.0 = disabled).
        """
    ...
    def _generate_and_score_completions(
        ...
        return {
            ...
            # TRL >= 0.24 _compute_loss expects this for loss normalization.
            # Keep as a 0-dim tensor (not a Python int) so split_tensor_dict doesn't choke on it.
...
class DreamGRPOTrainer(DiffuGRPOTrainer):
    """
    DiffuGRPOTrainer adapted for Dream-v0 models.

    Dream differs from MDLM/LLaDA in two ways that affect log-prob computation:

    1. Right-shifted logits: Dream is trained AR-style, so raw logits[:, i]
       predict token i+1. A one-position right-shift aligns logits[:, i] with
       target token i before computing cross-entropy.

    2. Explicit position IDs: Dream uses RoPE whose embeddings depend on
       position_ids derived from the attention mask. Without these, left-padded
       sequences get wrong positional encodings.

    Generation (DreamSampler) and everything else is inherited from
    DiffuGRPOTrainer unchanged.
    """
