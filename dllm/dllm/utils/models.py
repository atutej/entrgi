...
def get_tokenizer(
    ...
    is_bert_family = "bert" in model_cls_name.lower()

    # ---------------- Model-specific customization ----------------
    if model_cls_name == "LLaDAModelLM":
...
{% endif %}
"""
    elif model_cls_name == "A2DLlamaLMHeadModel":
        tokenizer.add_special_tokens({"mask_token": "<|mask|>"})
        tokenizer.eot_token = "<|eot_id|>"
        tokenizer.eot_token_id = tokenizer.convert_tokens_to_ids(tokenizer.eot_token)
    elif model_cls_name in {"A2DQwen2LMHeadModel", "A2DQwen3LMHeadModel"}:
        tokenizer.add_special_tokens({"mask_token": "<|mask|>"})
        tokenizer.eot_token = "<|im_end|>"
        tokenizer.eot_token_id = tokenizer.convert_tokens_to_ids(tokenizer.eot_token)
        ...
        print_main(
            "no tokenizer customization for model class:",
            model_cls,
            "from module:",
            model_cls_module,
        )
